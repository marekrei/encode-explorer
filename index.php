...
// The format is: array(username, hash, status)
// Status can be either "user" or "admin". User can read the page, admin can upload and delete.
// For example: $_CONFIG['users'] = array(array("username1", "hash1", "user"), array("username2", "hash2", "admin"));
...
//
// For example: $_CONFIG['users'] = array(array("username", "$2a$06$UMj/44z3r.SsaSR/70ylAu/kkpVXEbirvRedw6EWhxObl5oWqJDcm", "admin"));
...
class GateKeeper
{
	...
	public static function isUser($userName, $userPass)
	{
		foreach(EncodeExplorer::getConfig("users") as $user)
		{
			if($user[0] == $userName)
			{
				return $user[1] == crypt($userPass, $user[1]);
			}
		}
...
class EncodeExplorer
{
	...
	public function printLoginBox()
	{
		...
		<form enctype="multipart/form-data" action="<?php print $this->makeLink(false, false, null, null, null, ""); ?>" method="post">
			...
			<input name="user_name" id="user_name" type="text" /></div>
			<div><label for="user_pass"><?php print $this->getString("password"); ?>:</label>
			<input name="user_pass" id="user_pass" type="password" /></div>
